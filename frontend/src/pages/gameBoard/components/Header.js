...
const Header = ({colorMap, borderColorMap, room, countdown, turnType, handleJoinTeam}) => {
    ...
    return (<div className={"header"}>
        ...
        <ReadyButton turnType={turnType} countdown={countdown} room={room}/>
        <div className="team-container">
            ...
            {(!room.started && !isInSpectators()) ?
                <div onClick={() => handleJoinTeam(-1)} style={{"--fill-color": "#4c4c4c"}}
                     className={"join-team-button"}>Join</div> : null}
        </div>
    </div>)
}
